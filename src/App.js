...
function App() {
  ...
  // Belirtilen sembolün bir operatör (çarpma, bölme, toplama, çıkarma) olup olmadığını kontrol eden fonksiyon.
  ...
  const buttonPress = (symbol) => {
    if (symbol === "clear") {
      ...
      // "." butonuna tıklanınca ondalık nokta ekler, ancak bir sayıda birden fazla ondalık nokta eklenmesini engeller.
  ...
  const calculate = () => {
    // Son karakter bir operatörse, hesaplama işlemi yapmaz.
    if (isOperator(et.charAt(et.length - 1))) return;
    // İfadenin içinde iki operatör ardışık olarak bulunuyorsa, en son operatörü kullanarak hesaplama yapar.
    // 5 * - + 5 = 10
    const parts = et.split(" ");
    const newParts = [];

    // İfadelerin tersten incelenerek operatörlerin düzenlenmesi.
  ...
  return (
    <>
      <div className="container">
        <h1 style={{ color: "#fff"}}>Calculator Application</h1>
        <div id="calculator">
